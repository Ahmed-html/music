...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] || !args[1])
      return client.sendTime(message.channel, "❌ Invalid arguments, do something right in your life.");

    // Check if (args[0] - 1) is a valid index
    let trackNum = parseInt(args[0] - 1);
    if (trackNum < 1 || trackNum > player.queue.length - 1) {
      return client.sendTime(message.channel, "❌ Invalid track number, do something right in your life.");
    }
    ...
    if (dest < 1 || dest > player.queue.length - 1) {
      return client.sendTime(
        message.channel,
        "❌ Invalid track destination, do something right in your life."
      );
    }
